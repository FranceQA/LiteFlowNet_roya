...
class Matching(torch.nn.Module):
    def __init__(self, intLevel):
        ...
        self.netMain = torch.nn.Sequential(
            ...
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
            torch.nn.Conv2d(in_channels=32, out_channels=2, kernel_size=[0, 0, 7, 5, 5, 3, 3][intLevel], stride=1,
...
class Subpixel(torch.nn.Module):
    def __init__(self, intLevel):
        ...
        self.netMain = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=[0, 0, 130, 130, 194, 258, 386][intLevel], out_channels=128, kernel_size=3,
                            stride=1, padding=1),
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
            torch.nn.Conv2d(in_channels=128, out_channels=64, kernel_size=3, stride=1, padding=1),
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
...
class Regularization(torch.nn.Module):
    def __init__(self, intLevel):
        ...
        if intLevel >= 5:
            ...
            self.netFeat = torch.nn.Sequential(
                torch.nn.Conv2d(in_channels=[0, 0, 32, 64, 96, 128, 192][intLevel], out_channels=128, kernel_size=1,
                                stride=1, padding=0),
                torch.nn.LeakyReLU(inplace=False, negative_slope=0.1)
            )
        ...
        self.netMain = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=[0, 0, 131, 131, 131, 131, 195][intLevel], out_channels=128, kernel_size=3,
                            stride=1, padding=1),
            torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
        ...
        self.netScaleX = torch.nn.Conv2d(in_channels=[0, 0, 49, 25, 25, 9, 9][intLevel], out_channels=1, kernel_size=1,
                                         stride=1, padding=0)
        self.netScaleY = torch.nn.Conv2d(in_channels=[0, 0, 49, 25, 25, 9, 9][intLevel], out_channels=1, kernel_size=1,
                                         stride=1, padding=0)

    # eny

    def forward(self, tenFirst, tenSecond, tenFeaturesFirst, tenFeaturesSecond, tenFlow):
        tenDifference = (tenFirst - backwarp(tenInput=tenSecond, tenFlow=tenFlow * self.fltBackward)).pow(2.0).sum(1,
                                                                                                                   True).sqrt().detach()
...
class Network(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        tenFeaturesFirst = self.netFeatures(tenFirst)    #tenFeaturesFirst[F1,F2,F3,F4 F5 F6]
        ...
        # end tenFirst=[F0 F1 F2 F3 F4 F5 ]
        ...
        for intLevel in [ -1, -2, -3, -4, -5 ]:
            ...
            tenFlows += [tenFlow]
            outputs = torch.nn.functional.interpolate(input=tenFlow, size=(tenFirst[0].shape[2], tenFirst[0].shape[3]),
                                                      mode='bilinear', align_corners=False)
            tenFlows.append(outputs)
        # end
